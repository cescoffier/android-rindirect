...
public class VerboseTest {
    ...
    public static final File ROOT = new File("target/test-classes/verbose");
    ...
    @Test
    public void testVerbose() throws VerificationException, FileNotFoundException, IOException {
        Verifier verifier  = new Verifier( ROOT.getAbsolutePath() );
        Properties props = Constants.getSystemProperties();
        props.put("rindirect.verbose", "true");
        ...
        verifier.verifyTextInLog("INFO: Lauch R visit"); // Written in VERBOSE mode
    }

    @Test
    public void testNotVerbose() throws VerificationException, FileNotFoundException, IOException {
        Verifier verifier  = new Verifier( ROOT.getAbsolutePath() );
        Properties props = Constants.getSystemProperties();
        props.put("rindirect.verbose", "false");
        verifier.setSystemProperties(props);

        verifier.setLogFileName("log-not-verbose.txt");
        ...
        try {
            verifier.verifyTextInLog("INFO: Lauch R visit"); // Written in VERBOSE mode
            Assert.fail("Unexpected text in log");
